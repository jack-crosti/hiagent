...
import {
  DollarSign, TrendingUp, Receipt, Target, ArrowRightLeft,
  Plus, Sparkles
} from 'lucide-react';
...
export default function Dashboard() {
  ...
  async function loadStats() {
    ...
    const { data: gstPeriods } = await supabase
      .from('gst_periods')
      .select('net_gst, status')
      .eq('owner_user_id', user!.id)
      .eq('status', 'open');

    const income = txns?.filter(t => t.type === 'credit').reduce((s, t) => s + Number(t.amount), 0) ?? 0;
    const expenses = txns?.filter(t => t.type === 'debit').reduce((s, t) => s + Math.abs(Number(t.amount)), 0) ?? 0;
    const gst = gstPeriods?.reduce((s, g) => s + Number(g.net_gst), 0) ?? 0;
    const pipeline = deals?.reduce((s, d) => s + Number(d.net_to_user_after_tax) * Number(d.probability), 0) ?? 0;

    const hasData = (deals?.length ?? 0) > 0 || (txns?.length ?? 0) > 0;
  ...
  return (
    <>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 mb-6">
        <StatCard
          title="Total Income"
          value={formatNZD(stats.totalIncome)}
          icon={<DollarSign size={20} />}
          trend={stats.totalIncome > 0 ? { value: 'This period', positive: true } : undefined}
        />
        <StatCard
          title="Total Expenses"
          value={formatNZD(stats.totalExpenses)}
          icon={<ArrowRightLeft size={20} />}
        />
        <StatCard
          title="Pending GST"
          value={formatNZD(stats.pendingGst)}
          icon={<Receipt size={20} />}
          subtitle="Current period"
        />
        <StatCard
          title="Pipeline Value"
          value={formatNZD(stats.dealsPipeline)}
        ...
        <Card className="shadow-card">
          ...
          <CardContent className="space-y-2">
            ...
            <Link to="/personal-finance" className="block">
              <Button variant="outline" className="w-full justify-start">
                <Target size={16} className="mr-2" /> Create Deal
              </Button>
            </Link>
            <Link to="/gst" className="block">
              <Button variant="outline" className="w-full justify-start">
                <Receipt size={16} className="mr-2" /> Check GST Status
              </Button>
            </Link>
          </CardContent>
        </Card>
...
function DemoModeCard({ userId, onComplete }: { userId: string; onComplete: () => void }) {
  const [loading, setLoading] = useState(false);

  async function seedDemoData() {
    setLoading(true);
    try {
      // Seed demo bank account
      const { data: bankAcct } = await supabase.from('bank_accounts').insert({
        owner_user_id: userId,
        bank_name: 'ANZ',
        account_name: 'Business Cheque',
        account_number: '01-1234-5678900-00',
        account_type: 'cheque',
        balance: 45230,
        is_demo: true,
      }).select().single();
      ...
      await supabase.from('transactions').insert(
        txns.map(t => ({
          ...
          ...t,
          gst_amount: Math.abs(t.amount) * 0.15 / 1.15,
          is_demo: true,
        }))
      );
      ...
      await supabase.from('deals').insert([
        {
          owner_user_id: userId,
          deal_type: 'business_sale',
          listing_name: 'Café & Restaurant - Ponsonby',
          expected_close_date: '2026-04-15',
          probability: 0.7,
          sale_price: 850000,
          commission_rule_id: bizRule?.id,
          gross_commission_excl_gst: 76500,
          gst_on_commission: 11475,
          on_top_fee: 3000,
          commission_after_fee: 73500,
          user_share_excl_gst: 55125,
          company_share_excl_gst: 18375,
          estimated_tax: 18191,
          net_to_user_after_tax: 36934,
          is_demo: true,
        },
        {
          owner_user_id: userId,
          deal_type: 'business_sale',
          listing_name: 'Automotive Workshop - Hamilton',
          expected_close_date: '2026-05-30',
          probability: 0.45,
          sale_price: 1500000,
          commission_rule_id: bizRule?.id,
          gross_commission_excl_gst: 125000,
          gst_on_commission: 18750,
          on_top_fee: 3000,
          commission_after_fee: 122000,
          user_share_excl_gst: 91500,
          company_share_excl_gst: 30500,
          estimated_tax: 30195,
          net_to_user_after_tax: 61305,
          is_demo: true,
        },
        {
          owner_user_id: userId,
          deal_type: 'lease',
          listing_name: 'Office Space - 45 Queen St',
          expected_close_date: '2026-03-20',
          probability: 0.85,
          annual_rent_excl_gst: 120000,
          commission_rule_id: leaseRule?.id,
          gross_commission_excl_gst: 20000,
          gst_on_commission: 3000,
          on_top_fee: 1600,
          commission_after_fee: 18400,
          user_share_excl_gst: 14720,
          company_share_excl_gst: 3680,
          estimated_tax: 4858,
          net_to_user_after_tax: 9862,
          is_demo: true,
        },
      ]);
      ...
      await supabase.from('goal_plans').insert({
        owner_user_id: userId,
        name: 'FY 2025-26 Net Goal',
        target_net_amount: 200000,
